 ...
 * try {
 *     Class&lt;MyInterface&gt; newClass = compiler.compile(&quot;com.mypackage.NewClass&quot;, stringContaininSourceForNewClass, diagnostics,
 * 	    MyInterface);
...
public class CharSequenceCompiler<T> {
    ...
    public synchronized CompilationUnitTree parse(final String qualifiedClassName, final CharSequence javaSource,
	    final DiagnosticCollector<JavaFileObject> diagnosticsList, final Class<?>... types)
	    throws CharSequenceCompilerException, ClassCastException {
	if (diagnosticsList != null)
	    diagnostics = diagnosticsList;
	...
	Map<String, CharSequence> classes = new HashMap<String, CharSequence>(1);
	classes.put(qualifiedClassName, javaSource);
	List<CompilationUnitTree> parsed = parse(classes, diagnosticsList);
	return parsed.get(0);
    }
    ...
    public synchronized List<CompilationUnitTree> parse(final Map<String, CharSequence> classes,
	    final DiagnosticCollector<JavaFileObject> diagnosticsList) throws CharSequenceCompilerException {
	List<JavaFileObject> sources = new ArrayList<JavaFileObject>();
	for (Entry<String, CharSequence> entry : classes.entrySet()) {
	    String qualifiedClassName = entry.getKey();
	...
	try {
	    parseResult = ((JavacTask) task).parse();
	    List<CompilationUnitTree> compiled = new ArrayList<CompilationUnitTree>();
	    for (CompilationUnitTree cu : parseResult) {
		compiled.add(cu);
	    }
	    return compiled;
	} catch (IOException e) {
